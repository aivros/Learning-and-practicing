...
def dataframe_index():
    ...
    def xx_v(values):  #The parameter of the map function is used as a condition for generating a new index column.int64,float64.
        if values <= xx[0]:
            return xx[0]
        elif values <= xx[1]:
            return xx[1]
    ...
    for c_key in index_csv_dict:  #Based on the original data column, the loop generates a new index column.
        ...
        for tm in index_csv_dict[c_key]:
            ...
            if df_dataframe_csv[c_key][tm].dtypes == 'datetime64[ns]':                
                df_dataframe_csv[c_key][tm + '_year'] = df_dataframe_csv[c_key][tm].dt.year.astype('object')
                index_csv_main_dict[c_key][tm][tm + '_year'] =list(set(list(df_dataframe_csv[c_key][tm + '_year'].dropna())))
                df_dataframe_csv[c_key][tm + '_month'] = df_dataframe_csv[c_key][tm].dt.month.astype('object')
                index_csv_main_dict[c_key][tm][tm + '_month'] = list(set(list(df_dataframe_csv[c_key][tm + '_month'].dropna())))                
                df_dataframe_csv[c_key][tm + '_day'] = df_dataframe_csv[c_key][tm].dt.day.astype('object')
                index_csv_main_dict[c_key][tm][tm + '_day'] = list(set(list(df_dataframe_csv[c_key][tm + '_day'].dropna())))
                df_dataframe_csv[c_key][tm + '_hour'] = df_dataframe_csv[c_key][tm].dt.hour.astype('object')
                index_csv_main_dict[c_key][tm][tm + '_hour'] = list(set(list(df_dataframe_csv[c_key][tm + '_hour'].dropna())))
                df_dataframe_csv[c_key][tm + '_week'] = df_dataframe_csv[c_key][tm].dt.weekday_name
                index_csv_main_dict[c_key][tm][tm + '_week'] = list(set(list(df_dataframe_csv[c_key][tm + '_week'].dropna())))
            elif df_dataframe_csv[c_key][tm].dtypes == 'int64':
                xx = list(df_dataframe_csv[c_key][tm].quantile([.25, .5, .75,1]))#Quartile index.
                df_dataframe_csv[c_key][tm + '_int64'] = df_dataframe_csv[c_key][tm].map(xx_v)
                index_csv_main_dict[c_key][tm][tm + '_int64'] = list(set(list(df_dataframe_csv[c_key][tm + '_int64'].dropna())))
            elif df_dataframe_csv[c_key][tm].dtypes == 'float64':
                xx = list(df_dataframe_csv[c_key][tm].quantile([.31415926, .61803398875, .718281828,1]))#Mysterious index^_^.
                df_dataframe_csv[c_key][tm + '_float64'] = df_dataframe_csv[c_key][tm].map(xx_v)
                index_csv_main_dict[c_key][tm][tm + '_float64'] = list(set(list(df_dataframe_csv[c_key][tm + '_float64'].dropna())))
                ...
                df_dataframe_csv[c_key][tm + '_object'] = df_dataframe_csv[c_key][tm].map(xxx)#Initial index.
...
def out_print_count(csv_main,parameters_column,condition,goal,plot_data): 
    ...
    if parameters_column == []:
        ...
        plot_data_p.plot(kind = 'bar', color = color_x())
    else:        
        plt.figure(figsize=(15,8))
        plot_data.groupby(parameters_column)[goal].value_counts(normalize = False,sort = True).iloc[:31].plot(kind = 'bar',color = color_x())#iloc[:31]:Limit the output range to avoid rendering stuck.
        only_count = '<= 31'
...
def out_print_value(csv_main,parameters_column,condition,goal,plot_data):
    ...
    if parameters_column == []:
        ...
        df.plot(x= goal, y= 'mean', kind = 'bar', color = color_x())
    else:        
        plt.figure(figsize=(15,8))
        plot_data.groupby(parameters_column)[goal].mean().plot(kind = 'bar', color = color_x())
    ylavel_print = goal
